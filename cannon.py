...
class Shell(GameObject):
    ...
    def move(self, time=1, grav=0):
        ...
        self.vel[1] += grav
        for i in range(2):
            self.a[i] += time * self.vel[i]
        self.check_corners()
        if self.vel[0] ** 2 + self.vel[1] ** 2 < 2 ** 2 and self.a[1] > SCREEN_SIZE[1] - 2 * self.rad:
            self.is_alive = False
    ...
    def draw(self, screen):
        ...
        '''
        pg.draw.circle(screen, self.color, self.a, self.rad)

class Shell_type1(GameObject):
    '''
    The ball class. Creates a ball, controls it's movement and implement it's rendering.
    '''
    ...
    def __init__(self, coord, vel, rad=20, color=None):
        ...
        '''
        adjust = 3
        coord_new = [coord[0]-adjust , coord[1]-adjust]
        self.a = coord_new
        self.vel = vel
        if color == None:
            color = rand_color()
    ...
    def draw(self, screen):
        ...
        Draws the ball on appropriate surface.
        '''
        pg.draw.polygon(screen, self.color,[(self.a[0],self.a[1]), (self.a[0]+self.rad, self.a[1]),(self.a[0]+self.rad, self.a[1]+self.rad),(self.a[0], self.a[1]+self.rad)] )
...
class Cannon(GameObject):
    ...
    def strike_type1(self):
        '''
        Creates ball, according to gun's direction and current charge power.
        '''
        vel = self.pow
        angle = self.angle
        ball = Shell_type1(list(self.coord), [int(vel * np.cos(angle)), int(vel * np.sin(angle))])
        self.pow = self.min_pow
        self.active = False
        return ball
...
class Target(GameObject):
    ...
    def check_collision(self, ball):
        ...
        '''
        dist = sum([(self.coord[i] - ball.a[i]) ** 2 for i in range(2)]) ** 0.5
...
class Target_type1(GameObject):
    ...
    def __init__(self, coord=None, color=None, rad=30):
        ...
        '''
        if coord == None:
            a = [randint(0, SCREEN_SIZE[0] - rad), randint(0, SCREEN_SIZE[0] - rad)]
            self.a = a
        self.rad = rad
    ...
    def check_collision(self, ball):
        ...
        Checks whether the ball bumps into target.
        '''
        dist = sum([(self.a[i]+self.rad/2 - ball.a[i]) ** 2 for i in range(2)]) ** 0.5
        min_dist = self.rad + ball.rad
        return dist <= min_dist
...
class MovingTarget_type1(Target_type1):
    def __init__(self, coord=None, color=None, rad=30):
        super().__init__(coord, color, rad)
        self.vx = randint(-2, +2)
        self.vy = randint(-2, +2)
    ...
    def move(self):
        self.a[0] += self.vx
        self.a[1] += self.vy
        if self.a[0] < self.rad or self.a[0] > SCREEN_SIZE[0] - self.rad:
            self.vx *= -1
        if self.a[1] < self.rad or self.a[1] > SCREEN_SIZE[1] - self.rad:
            self.vy *= -1
    ...
    def new_mission(self):
        ...
        for i in range(self.n_targets):
            self.targets.append(MovingTarget(rad=randint(max(1, 30 - 2 * max(0, self.score_t.score())),
                                                         30 - max(0, self.score_t.score()))))
        for i in range(self.n_targets):
            self.targets.append(Target_type1(rad=randint(max(1, 30 - 2 * max(0, self.score_t.score())),
                                                         30 - max(0, self.score_t.score()))))
        for i in range(self.n_targets):
            self.targets.append(MovingTarget_type1(rad=randint(max(1, 30 - 2 * max(0, self.score_t.score())),
                                                         30 - max(0, self.score_t.score()))))
    ...
    def handle_events(self, events):
        ...
        for event in events:
            if event.type == pg.QUIT:
                ...
                if event.button == 1:
                    if self.score_t.b_used%2 == 0:
                        self.balls.append(self.gun.strike())
                    else:
                        self.balls.append(self.gun.strike_type1())
